...
Future<void> getOfficialNoticeData(index) async {
  final ConnectivityController connectivityController =
      Get.put(ConnectivityController()); // 네트워크 연결체크
  final NoticeDataController noticeDataController =
      Get.put(NoticeDataController());
    ...
    try {
      if (response.statusCode == 200) {
        ...
        if (resultList[0]["result"] == null) {
          OfficialNoticeData officialNoticeData =
              OfficialNoticeData.fromJson(resultList[0]);
          final OfficialNoticeDataController officialNoticeDataController =
              Get.put(OfficialNoticeDataController());
          officialNoticeDataController
              .setOfficialNoticeData(officialNoticeData);
        } else {}
      }
    }
    // 응답을 받지 못했을 때
